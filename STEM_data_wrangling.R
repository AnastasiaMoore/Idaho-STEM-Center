...
#create character columns to display the text in tableau for median wages columns (take care of >=$100 of wage & 208000)
...
occupations_data <- occupations_data %>% 
  mutate(Text_Median_Hourly_Wage_column = ifelse(eval(parse(text=Median_Hourly_Wage_column)) == 110, ">=$100.0", paste0("$", round(eval(parse(text=Median_Hourly_Wage_column)),2))))


Median_Annual_Wage_column <- names(occupations_data)[which(grepl("Median_Annual_Wage_2", names(occupations_data)))]

occupations_data <- occupations_data %>% 
  mutate(Text_Median_Annual_Wage_column = ifelse(eval(parse(text=Median_Annual_Wage_column)) == 208100, ">=$208,000", paste0("$", comma_format()(eval(parse(text=Median_Annual_Wage_column))))))
